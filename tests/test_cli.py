...
@pytest.fixture
def yaml_breaking_template_file(tmp_path):
    """Create a template that might produce YAML-breaking sequences."""
    template = tmp_path / "yaml_breaking.jinja"
    template.write_text("""
    {% llmquery model="gpt-4", stream=true %}
    Generate a short YAML example with --- and ... markers.
    {% endllmquery %}
    """)
    return template
...
@patch('jinja_prompt_chaining_system.parser.LLMClient')
@patch('jinja_prompt_chaining_system.parser.LLMLogger')
def test_cli_yaml_breaking_sequences(mock_logger, mock_llm_client, mock_render, runner, yaml_breaking_template_file, context_file):
    """Test CLI with content that contains YAML-breaking sequences."""
    # Setup mock with complete response
    yaml_output = """Here's a YAML example:
---
name: John
age: 30
...
"""
    mock_render.return_value = yaml_output
    
    with runner.isolated_filesystem():
        template_path = os.path.join(os.getcwd(), "yaml_breaking.jinja")
        context_path = os.path.join(os.getcwd(), "context.yaml")
        log_dir = os.path.join(os.getcwd(), "logs")
        ...
        with open(yaml_breaking_template_file, "rb") as f:
            with open(template_path, "wb") as tf:
                tf.write(f.read())
    ...
    # Verify the output contains the YAML content
    assert "Here's a YAML example:" in result.output
